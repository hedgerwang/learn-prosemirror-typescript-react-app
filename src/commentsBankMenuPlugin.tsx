...
export default function commentsBankMenuPlugin(): Plugin {
  const plugin = new EditorViewPlugin({
    ...
    state: {
      ...
      apply(tr: Transaction, set: DecorationSet): DecorationSet {
        ...
        // Remove menu.
        if (action[HIDE_COMMENTS_BANK_MENU] || tr.docChanged) {
          set = set.remove(decos);
          reactRoot && reactRoot.unmount();
          plugin.reactRoot = null;
        }

        if (action[SHOW_COMMENTS_BANK_MENU]) {
          if (selection instanceof TextSelection && editorView) {
            ...
            root.render(
              <React.StrictMode>
                <CommentsBankMenu editorView={editorView} />
              </React.StrictMode>
            );
